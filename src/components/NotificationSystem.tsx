...
export interface Toast {
  ...
  title: string;
  message: string;
  type: 'security' | 'success' | 'warning' | 'info' | 'error';
  duration?: number;
}
...
const ToastItem: React.FC<{ toast: Toast; onRemove: () => void }> = ({ toast, onRemove }) => {
  ...
  useEffect(() => {
    const hideTime = toast.duration || 5000;
    timerRef.current = setTimeout(triggerDismiss, hideTime);
  ...
  const configMap = {
    ...
    error: { icon: ShieldAlert, color: 'text-rose-500', bg: 'bg-rose-500/10', border: 'border-rose-500/20' },
  };
  ...
  return (
    <div 
      className={`pointer-events-auto group relative overflow-hidden backdrop-blur-3xl border ${config.border} ${config.bg} p-6 rounded-[2.5rem] shadow-2xl transition-all duration-400 ${
    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1 pr-6">
          <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white">{toast.title}</h4>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">{toast.message}</p>
        </div>
        <button 
          onClick={triggerDismiss} 
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 dark:hover:text-white p-1 transition-colors"
        >
          <X size={16} />
        </button>
      </div>
